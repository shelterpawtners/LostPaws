...
test.describe("Issue #125 RAVE Shelter and LostPaws launch surfaces", () => {
  test("keeps RAVE Shelter and LostPaws intentionally distinct", async ({ page }) => {
    await page.goto("/rave");
    await expect(page.locator(".rsmPage")).toBeVisible();
    await expect(page.getByText("Rescue and Adoption Vendor Ecosystem", { exact: true })).toBeVisible();
    await expect(page.locator(".lpPage")).toHaveCount(0);

    await page.goto("/lostpaws");
    await expect(page.locator(".lpPage")).toBeVisible();
    await expect(page.getByText("A RAVE Shelter activation", { exact: true })).toBeVisible();
    await expect(page.getByRole("heading", { name: "Bring the mission into the music community." })).toBeVisible();
    await expect(page.locator(".rsmPage")).toHaveCount(0);
  });

  for (const width of mobileWidths) {
    test(`keeps launch-critical campaign pages inside a ${width}px viewport`, async ({ page }) => {
      ...
      for (const route of ["/rave", "/lostpaws"]) {
        ...
        expect(sizes.scroll, `${route} @ ${width}px`).toBeLessThanOrEqual(sizes.client);
      }
    });
  }

  test("preserves the full LostPaws hero artwork on mobile and keeps primary actions touchable", async ({ page }) => {
