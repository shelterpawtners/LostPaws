...
CURRENT_PHASE: Vercel deployment-classifier fix + Marketplace/footer compact redesign — merged
CURRENT_CHECKPOINT: PR #137 merged to `main` (squash commit `c013203`), fast-forwarding past PR #135 (Track 1-3, `27c07d0`) and Codex's parallel docs/Seven-Star/legal work. Three parts:

1. **Vercel deployment-classifier fix.** Root cause: `scripts/vercel-ignore-build.sh` diffed only `HEAD^` vs `HEAD`, so a product merge (PR #135) followed by docs-only commits looked docs-only in every later commit's own one-step diff, and Vercel kept skipping the build even though production was still behind. Reproduced directly against git history before touching the script. Fixed by comparing against `$VERCEL_GIT_PREVIOUS_SHA` (the commit the prior deployment actually built from), failing toward building when that variable is unset or unresolvable. Added `scripts/verify-vercel-ignore-build.sh` (a throwaway-temp-repo regression suite covering all 4 required cases plus both explicit markers; fails 3/6 against the pre-fix script, passes 6/6 against the fix), wired into `npm run check` and CI as `npm run test:shell`. Note for whoever can check the Vercel dashboard: this PR's own preview build showed "Canceled by Ignored Build Step" on every push despite containing real product changes — could not verify from here whether `VERCEL_GIT_PREVIOUS_SHA` behaves differently for preview vs. production contexts (a real prior preview already capturing the diff would make this correct; if not, it's a related gap worth a follow-up). Does not block anything — not a required merge gate.
2. **Marketplace compact redesign.** First offer card now at 275px desktop / 387px mobile (was 1171px / 1387px) — replaced the hero/value-panel/stacked-filters with one compact header + toolbar (title+count, All/Pet/RAVE segmented control, search+category+type+sort+grid/list toggle, one trust line, mobile-only filter disclosure).
3. **Footer compact redesign.** One row of four columns on desktop instead of two wrapped rows; 2-column mobile wrap with the brand spanning full width. Existing 44px link touch-target floor untouched.

Verified together: `npm run check`, the full public e2e suite (59 tests), 48 persona e2e tests, a normal production build, and a `GITHUB_PAGES=true` build all pass/succeed. `main` is green post-merge (CI Gate, GitHub Pages Staging). Two `phase-1-accessibility.spec.ts` failures (mobile-menu, RAVE-logo) reproduce on a clean run but touch no file this PR changed relative to `main` — confirmed pre-existing, flagged, not fixed (out of scope for this sprint). Did not touch Meta/Facebook, Microsoft 365 DNS, secrets, money movement, or the sign-out race, per the sprint brief's guardrails.
NEXT_CHECKPOINT: Next normal Vercel production deployment from `main` should now build (classifier fixed). No owner action required unless the Vercel-preview note above turns out to need a follow-up.
OWNER_DECISION_REQUIRED: NO
SAFE_TO_CONTINUE: YES
ACCEPTED_PRODUCT_SHA: c013203d5b43684b0894c614642e0feb1db9a4c9
