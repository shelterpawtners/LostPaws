...
CURRENT_PHASE: Vercel deployment-classifier fix + Marketplace/footer compact redesign, on `fix/vercel-deploy-classifier`
CURRENT_CHECKPOINT: Tracks 1 and 2 merged to `main` (PR #130 `07ba8f1`, PR #131 `9c114ea`). `feature/marketplace-ux-and-giving` (PR #135) adds: site-wide scroll-to-top on navigation, a shortened LostPaws page, a marketplace redesign (real tags/images, category filter, sort, tighter mobile layout), a business giving panel and Guardian giving history built on the pre-existing donation schema, a business-facing tax/giving document, new Learn articles, a written (not yet built) proposal for offer link-preview image fetching, a RAVE Shelter nav dropdown (holding the mission page, LostPaws, and Seven Star Shelters), a fix for LostPaws' duplicated logo, and new Terms/Data Deletion placeholder pages linked from the footer alongside the Privacy Policy a parallel agent published this session. Typecheck, lint, 50 unit tests, 59 public e2e tests, and 48 persona e2e tests all pass locally against a freshly reset database. Also fixed, directly on `main` (commit `3abebc3`): a prettier-formatting regression in `docs/AI-CONTROLLER.md` that was failing `main`'s CI Gate, introduced by a docs-only commit from another agent.
NEXT_CHECKPOINT: `fix/vercel-deploy-classifier` pushed, not yet a PR. Three parts, each its own commit(s):

1. **Vercel deployment-classifier fix.** Root cause: `scripts/vercel-ignore-build.sh` diffed only `HEAD^` vs `HEAD`, so PR #135 (a real product merge) followed by docs-only commits looked docs-only in every later commit's own one-step diff, and Vercel kept skipping the build even though production was still behind. Reproduced directly against git history before touching the script (`classify-change-impact.sh <commit-before-135> HEAD` correctly reports `frontend_artifact=true`; `HEAD^ HEAD` at any later docs-only commit reports `false`). Fixed by comparing against `$VERCEL_GIT_PREVIOUS_SHA` (the commit the prior deployment actually built from) instead, failing toward building when that variable is unset or unresolvable. Added `scripts/verify-vercel-ignore-build.sh`, a self-contained regression suite that builds a throwaway temp repo and covers all 4 required cases plus both explicit markers; confirmed it fails 3/6 checks against the reverted pre-fix script (negative control) and passes 6/6 against the fix. Wired into `npm run check` and the CI `web` job as `npm run test:shell`.
2. **Marketplace compact redesign.** Replaced ~1170-1390px of hero copy, a value-breakdown panel, and four stacked filter blocks with one compact block (title+count, an always-visible All/Pet/RAVE segmented control, one search+filter toolbar, one trust line). First card now at 275px desktop / 387px mobile (was 1171px / 1387px). Fixed two regressions the class rename caused (a `:has()` selector hiding the duplicate legacy hero, and grid/list card styling that was scoped to a class the new markup had dropped — kept both classes together) and one it introduced (two sub-44px touch targets, and a duplicate `<h1>`, both fixed).
3. **Footer compact redesign.** One row of four columns on desktop instead of two wrapped rows; 2-column wrap on mobile with the brand spanning full width instead of one full-height column at a time. Padding and gaps reduced; the existing 44px link touch-target floor is untouched.

Verified together: `npm run check` (lint, shell syntax, the new classifier regression suite, typecheck, unit tests) all green; the full public e2e suite (59 tests) and 48 persona e2e tests pass on a freshly reset local database; both a normal production build and a `GITHUB_PAGES=true` build succeed with no console errors when the built app is served and exercised directly. Two phase-1-accessibility.spec.ts failures (mobile-menu, RAVE-logo) reproduce on a clean run but touch no file this branch changes relative to `main` — confirmed pre-existing, out of scope for this sprint, not fixed here. Did not touch Meta/Facebook, Microsoft 365 DNS, secrets, money movement, or the sign-out race, per the sprint brief's guardrails.
OWNER_DECISION_REQUIRED: NO
SAFE_TO_CONTINUE: YES
ACCEPTED_PRODUCT_SHA: a764f6c2d01a367d382acde8f7b2492a24e47a99
