...
NEXT_CHECKPOINT: PR #135 (https://github.com/shelterpawtners/LostPaws/pull/135) is CI-green (all required gates pass; Vercel remains Hobby-plan rate-limited, unrelated and non-blocking) and merging now under the standing merge authorization. A same-day attempt to fix an occasional CI-only sign-out race was tried, found to make things worse, and reverted — see `docs/OPEN-ITEMS-2026-09-12.md` §3.2b for the honest record; the revert is what's shipping.
