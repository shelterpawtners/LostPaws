...
NEXT_CHECKPOINT: PR #135 merged to `main` (squash commit `27c07d0`) under the standing merge authorization. `main` is fully green: after the merge, two more stale hardcoded assertions from earlier copy/UI changes surfaced and were fixed directly on `main` — `github-pages-staging.yml` asserted a home-page heading ("Music. Community...") that Track 1's brand correction had already replaced with "Rave with purpose. Shop with impact." (fixed in `40e1de1`), and `github-pages-mvp-acceptance.yml`'s live authenticated-Guardian smoke test still drove the old inline "Help & feedback" toggle that this session's Support page migration replaced with a plain "Help & support" link to `/support` (fixed in `a764f6c`). CI, GitHub Pages Staging, and GitHub Pages MVP Acceptance all pass on `main` as of `a764f6c`. A same-day attempt to fix an occasional CI-only sign-out race was tried, found to make things worse, and reverted before merge — see `docs/OPEN-ITEMS-2026-09-12.md` §3.2b for the honest record.
