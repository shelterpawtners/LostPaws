...
## Acceptance
Partner Admin -> `/business` -> edit public description -> Save draft -> success -> reload -> same organization remains selected -> saved description and required draft fields reload correctly.

Then continue the existing profile publish -> offer publish -> Guardian discovery -> claim/redemption golden path.
