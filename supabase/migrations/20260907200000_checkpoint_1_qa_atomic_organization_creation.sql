-- Checkpoint 1 QA: creation must not leave an organization behind when a
-- membership, location, relationship, or draft-resolution write fails.
...
-- Membership, not the historical creator column, is the enduring authority.
...
using (private.can_manage_org(id, array['owner','administrator']))
with check (
  private.can_manage_org(id, array['owner','administrator'])
