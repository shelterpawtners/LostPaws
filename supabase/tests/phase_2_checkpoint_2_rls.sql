...
select is(
  (with denied as (update public.organization_partner_profiles set public_description='takeover' where organization_id='20000000-0000-0000-0000-000000000001' returning 1) select count(*)::bigint from denied),
  0::bigint,
  'Partner B cannot edit Partner A profile'
);
select is(
  (with denied as (update public.organization_private_contacts set primary_contact_email='takeover@example.invalid' where organization_id='20000000-0000-0000-0000-000000000001' returning 1) select count(*)::bigint from denied),
  0::bigint,
...
select is((select count(*)::bigint from public.public_partner_profile_details('20000000-0000-0000-0000-000000000001')),0::bigint,'unpublished profile is absent from public details');
reset role;
...
set local role authenticated;
select set_config('request.jwt.claim.sub','10000000-0000-0000-0000-000000000006',true);
select is((with denied as (update public.organization_partner_profiles set public_description='revoked edit' where organization_id='20000000-0000-0000-0000-000000000003' returning 1) select count(*)::bigint from denied),0::bigint,'revoked former creator cannot edit its profile');
